import BulkLandPage from "app/bulk-land-in-dholera/page"
...
const MAIL_URL =
  process.env.NEXT_PUBLIC_MAIL_API_URL || 'https://www.smarthomesinfra.in'
...
const api = {
  ...
  CompletedPropertyList: async () => {
    ...
    const data = await fetchData.adminPost(
  ...
  CareerFileUpload: async (uploadData) => {
    const formData = new FormData()
    formData.append('from_app', uploadData.from_app)
    formData.append('upload_type', uploadData.upload_type)
    formData.append('resume_upload', uploadData.resume_upload)
