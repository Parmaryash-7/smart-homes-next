...
const BOOKING_URL = 'https://www.reecosys.com/api/Bookings/'
// const BOOKING_URL = 'http://192.168.1.100/reecosys/api/Bookings/'
const MAIL_URL =
...
export const fetchData = {
  ...
  bookingPost: async (url, data) => {
    // console.log(SERVICES_URL + url)
    try {
      const response = await fetch(BOOKING_URL + url, {
        method: 'POST',
        headers: {
          Authorization: `User ${getToken()}`,
          'Content-Type': 'application/json'
...
const api = {
  ...
  //Admin methods Start
  AdminDetail: async () => {
    const body = { ...masterIds, user_name: 'smarthomesinfrastructure' }
  ...
  PlotBooking: async (payload) => {
    const data = await fetchData.bookingPost(`/plot_booking`, payload)
    return data
  },
  unitTransaction: async (payload) => {
    const data = await fetchData.bookingPost(`/unit_transcation`, payload)
    return data
  },
  updatePaymentStatus: async (payload) => {
    const data = await fetchData.bookingPost(`/update_payment_status`, payload)
    return data
  }
