...
export default function BookPlotsForm({ propertylist = [], completedPropertylist = [], searchParams = {} }) {
    ...
    const validateForm = () => {
        ...
        if (!inquiryObj2.full_name.trim()) errors.full_name = "Full Name* is required";
        if (!inquiryObj2.so_do_wo.trim()) errors.so_do_wo = "S/O, D/O, W/O* is required";

        // Phone validation
        if (!inquiryObj2.phone_number || inquiryObj2.phone_number.trim().length !== 10 || !/^\d{10}$/.test(inquiryObj2.phone_number.trim())) {
        ...
        if (!inquiryObj2.date_of_birth_display.trim()) errors.date_of_birth_display = "Date of Birth* is required";
        if (!inquiryObj2.nationality) {
            errors.nationality = "Nationality* is required";
        } else {
            const selectedCountry = countryList.find(c => c.nicename === inquiryObj2.nationality);
            if (!selectedCountry) {
        ...
        if (!inquiryObj2.permanent_address.trim()) errors.permanent_address = "Permanent Address* is required";
        if (!inquiryObj2.current_address.trim()) errors.current_address = "Present Home Address* is required";
        if (!inquiryObj2.occupation.trim()) errors.occupation = "Profession* is required";
        if (!inquiryObj2.company_name.trim()) errors.company_name = "Company Name* is required";

        // Reference
        if (!inquiryObj2.reference_name?.trim()) errors.reference_name = "Reference Name* is required";
        if (!inquiryObj2.reference_contact_number || inquiryObj2.reference_contact_number.trim().length !== 10 || !/^\d{10}$/.test(inquiryObj2.reference_contact_number.trim())) {
            errors.reference_contact_number = 'Valid 10-digit reference number is required';
        }

        // Project details
        if (!inquiryObj2.project_type) errors.project_type = "Project Type* is required";
    ...
    const handleChange = (e) => {
        ...
        if (name === 'project_id') {
            const selectedProject = filteredProjects.find(p => p.project_id === value);
    ...
    const handlePlotSelect = (plotNumber) => {
        const selectedUnit = unitPlans.find((unit) => unit.flat_no_temp === plotNumber);
        if (selectedUnit) {
            setInquiryObj2((prev) => ({
                ...prev,
                ...
                <div className="mini-container">
                    <div className="">
                        <div className="inner-flex inner-flex-big">
                            ...
                            <form id="inquiry_form" name="inquiry_form" onSubmit={handleSubmit}>
                                <div className="form_wrapper">
                                    <div className="section-paragraph section-padding-half-ooo pb0">
                                        <p className="bold-fonts highlight-color">PERSONAL INFORMATION*</p>
                                    </div>

                                    <div className="personal-grid">
                                        <div className="contact_lable">
                                            <input id="full_name" name="full_name" type="text" className={`form-control ${formErrors.full_name ? 'error' : ''}`} tabIndex="101" autoComplete="off" value={inquiryObj2.full_name} onChange={handleChange} />
                                            <label className="md-lable" htmlFor="full_name">Full Name*</label>
                                        </div>

                                        <div className="contact_lable">
                                            <input id="so_do_wo" name="so_do_wo" type="text" className={`form-control ${formErrors.so_do_wo ? 'error' : ''}`} tabIndex="102" autoComplete="off" value={inquiryObj2.so_do_wo} onChange={handleChange} />
                                            <label className="md-lable" htmlFor="so_do_wo"> S/O, D/O, W/O*</label>
                                        </div>
                                        ...
                                        <div className="contact_lable relative">
                                            ...
                                            <label className="md-lable contact_code" htmlFor="phone_number">Mobile Number*</label>

                                            {/* Country Code Selector with Flag */}
                                            <div className="conatct_number_input">
                                                <div className="country_code_data" onClick={handleUserFlagClick}>
                                                ...
                                                <div className="country_code_list_data active" onClick={(e) => e.stopPropagation()}>
                                                    ...
                                                    <ul>
                                                        {filteredUserCountries.map((country, i) => (
                                                            <li key={i} onClick={() => handleUserCountrySelect(country.phonecode, country.flag)}>
                                                                <div>
                                                                    <img src={country.flag} alt={country.nicename} style={{ width: 20, height: 14, marginRight: 8 }} />
                                        ...
                                        <div className="contact_lable noPrint">
                                            <div className="relative">
                                                ...
                                                <div className="date_picker_input_text">
                                                    <p className={`date_picker_input_text_p ${bookPlotBirthDate ? 'active' : ''}`}>{bookPlotBirthDate || 'dd-mm-yyyy'}</p>
                                                    <span className="material-symbols-outlined" style={{ cursor: 'pointer' }}>calendar_today</span>
                                                </div>
                                                <label className="md-lable" htmlFor="date_of_birth">Date of Birth*</label>
                                            </div>
                                        </div>
                                        ...
                                        <div className="contact_lable select_option select_apr relative noPrint">
                                            <select
                                                ...
                                                onChange={handleChange}
                                            >
                                                <option value="" disabled className="gray-color">Nationality</option>
                                                {countryList.map((country, index) => (
                                                    <option key={index} value={country.nicename}>{country.nicename}</option>
                                                ))}
                                            </select>

                                            <label className="md-lable project_id_label" htmlFor="nationality">Nationality</label>
                                        ...
                                        <div className="contact_lable commentb  ox">
                                            <textarea
                                                ...
                                                onChange={handleChange}
                                            />
                                            <label className="md-lable" htmlFor="permanent_address">Permanent Address*</label>
                                        </div>
                                        ...
                                        <div className="contact_lable commentbox">
                                            <textarea
                                                ...
                                                onChange={handleChange}
                                            />
                                            <label className="md-lable" htmlFor="current_address">Present Home Address*</label>
                                        </div>
                                        ...
                                        <div className="contact_lable">
                                            <input
                                                ...
                                                onChange={handleChange}
                                            />
                                            <label className="md-lable" htmlFor="occupation">Profession*</label>
                                        </div>
                                        ...
                                        <div className="contact_lable">
                                            ...
                                            <input
                                                ...
                                                onChange={handleChange}
                                            />
                                            <label className="md-lable" htmlFor="company_name">Company Name*</label>
                                        </div>
                                        ...
                                        <div className="contact_lable">
                                            <input
                                                ...
                                                onChange={handleChange}
                                            />
                                            <label className="md-lable" htmlFor="work_address">Work Address*</label>
                                        </div>
                                    </div>
                                    <div className="plot-details-grid">
                                        <div className="contact_lable select_option select_apr relative noPrint">
                                            ...
                                            <label className="md-lable" htmlFor="project_type">Project Type*</label>
                                        ...
                                        <div className="contact_lable select_option select_apr relative noPrint">
                                            ...
                                            >
                                                ...
                                                {filteredProjects.map(project => (
                                                    <option key={project.project_id} value={project.project_id}>{project.project_title}</option>
                                                ))}
                                            </select>
                                            <label className="md-lable" htmlFor="project_id">Project Name*</label>
                                            {/* <div className="fa-icon"><i className="fa fa-angle-down" /></div> */}
                                            <span className="material-symbols-outlined fa-icon">
                                                keyboard_arrow_down
                                            </span>
                                        ...
                                        <div className="contact_lable select_option select_apr relative noPrint">
                                            ...
                                            >
                                                ...
                                                {unitPlans.map(unit => (
                                                    <option key={unit.unit_id} value={unit.flat_no_temp}>{unit.flat_no_temp}</option>
                                                ))}
                                            </select>
                                            <label className="md-lable" htmlFor="plot_number">Plot Number*</label>
                                            {/* <div className="fa-icon"><i className="fa fa-angle-down" /></div> */}
                                            <span className="material-symbols-outlined fa-icon">
                                                keyboard_arrow_down
                                            </span>
                                        ...
                                        <div className="contact_lable">
                                            ...
                                            <input
                                                ...
                                                readOnly
                                            />
                                            <label className="md-lable" htmlFor="plot_size">Plot Size</label>
                                        </div>
                                    </div>
                                    ...
                                    <div className="plot-details-grid">
                                        ...
                                        <div className="contact_lable relative">
                                            <input
                                                ...
                                                onChange={handleChange}
                                            />
                                            <label className="md-lable contact_code" htmlFor="reference_contact_number">
                                                Reference Mobile Number *
                                            </label>

                                            <div className="conatct_number_input">
                                                <div className="country_code_data" onClick={handleRefFlagClick}>
                                                    <div className="section-paragraph">
                                                        <span>
                                                ...
                                                <div className="country_code_list_data active" onClick={(e) => e.stopPropagation()}>
                                                    ...
                                                    <ul>
                                                        {filteredRefCountries.map((country, i) => (
                                                            <li key={i} onClick={() => handleRefCountrySelect(country.phonecode, country.flag)}>
                                    ...
                                    <div className="inner-flex inner-flex-smallest section-padding-half pb0">
                                        <div className="section-paragraph ">
                                            <p className="highlight-color bold-fonts uppercase">Declaration & Agreement</p>
                                        </div>
                                        <div className="section-content">
                                            <p>
                                                By signing this application for the plotting project, I hereby declare that
                                                all the
                                                information provided herein is true, accurate, and complete to the best of
                                                my knowledge.
                                                I understand that any misrepresentation or omission of facts may result in
                                                the re-
                                                jection of my application or, if discovered later, may lead to the
                                                termination of my
                                                participation in the project without any refund. I also acknowledge that I
                                                have read and
                                                understood all the terms and conditions associated with this application and
                                                agree to
                                                abide by them. Furthermore, I authorize the project management team to
                                                verify the
                                                information provided and to contact any necessary parties for this purpose.
                                            </p>
                                        </div>
                                    </div>

                                    {/* Terms & Conditions */}
                                    <p className={`relative check_box_error get_update_text align_center ${formErrors.terms_condition_agreed_display ? 'error' : ''}`}>
                                        <input
                                            ...
                                            tabIndex="120"
                                            checked={inquiryObj2.terms_condition_agreed_display || false}
                                            onChange={handleChange}
                                        />
                                        <label
                                            className={`darkgray-font ${formErrors.terms_condition_agreed_display ? 'error-label' : ''}`}
